...
description: >
  ...
  test in HintjumpCoreTests first, prove it fails, implement the minimum in
  HintjumpCore to pass, then refactor and re-verify the coverage floor. Use
...
- **Logic, state, view models** → `Packages/HintjumpKit/Sources/HintjumpCore` — this is
  the default. Core is coverage-gated, so code here is forced to stay tested.
- **Rendering only** → `Packages/HintjumpKit/Sources/HintjumpUI`, as a thin view over a
  Core view model. If a view needs an `if`, the condition belongs in Core.
- **Talking to the OS** → a `Sendable` port (protocol) in `HintjumpCore` plus its adapter
  in `Packages/HintjumpKit/Sources/HintjumpPlatform`. Test the Core side against a fake of
  ...
  `Packages/HintjumpKit/Tests/HintjumpPlatformTests` that you run by hand with
...
- If you are about to put logic in `HintjumpUI`, `HintjumpPlatform`, or `App/`, stop and move
  it to Core.

## Step 1: RED — Write the Failing Test First

Add the test to `Packages/HintjumpKit/Tests/HintjumpCoreTests` using Swift Testing:
...
```bash
cd Packages/HintjumpKit && swift test
```
...
Write the smallest implementation in `HintjumpCore` that makes the test pass.
...
```bash
just check   # fmt → lint → test (80% floor on HintjumpCore) → build
```
