...
- **more, tangled edges, or a close top-two** — hand
  ...
  independent `architect` sub-agent, filled per
...
default; `architect` when the issue is foundational or hard to carry out**
([when implementation goes to `architect`](references/cost-discipline.md#when-implementation-goes-to-architect));
...
run budget, sub-agent tiers, what parallel mode costs against what it
